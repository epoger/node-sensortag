...
CC2650SensorTag.prototype.readAccelerometerMultisample  = function(numSamples, callback) {
  this.readDataCharacteristic(MPU9250_UUID, MPU9250_DATA_UUID, function(error, data) {
    if (error) {
      return callback(error);
    }

    console.log('EPOGER data', data);
    var bytesPerSample = 6;
    var numSamplesReceived = data.length / bytesPerSample;

    var allSamples = [];
    for (var i=0; i<numSamplesReceived; i++) {
      var offset = bytesPerSample * i;
      // we specify 2G range in setup
      var x = data.readInt16LE(offset+0) * 2.0 / 32768.0;
      var y = data.readInt16LE(offset+2) * 2.0 / 32768.0;
      var z = data.readInt16LE(offset+4) * 2.0 / 32768.0;
      var oneSample = [x, y, z];
      allSamples.push(oneSample);
    }
    callback(null, allSamples);
  }.bind(this));
};
